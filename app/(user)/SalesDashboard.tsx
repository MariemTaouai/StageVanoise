...
export default function SalesDashboardScreen() {
  ...
  const fetchWithError = useCallback(
    async <T,>(url: string, fallback: T): Promise<T> => {
      try {
    ...
    async (url?: string) => {
      ...
      try {
        ...
        const [
          ...
          fetchWithError<BudgetWeeklyItem[]>(
            `${baseUrl}/api/sales/budget-weekly`,
            [],
          ),
          fetchWithError<ForecastWeeklyItem[]>(
            `${baseUrl}/api/sales/forecast-weekly`,
            [],
          ),
          fetchWithError<YearlyData>(
            `${baseUrl}/api/sales/yearly-volume-comparison`,
            {} as YearlyData,
          ),
          fetchWithError<MonthlyData[]>(
            `${baseUrl}/api/sales/monthly-history`,
            [],
          ),
        ]);
        ...
        if (Array.isArray(budgetWeeklyData) && budgetWeeklyData.length > 0) {
          const bVol = budgetWeeklyData.reduce(
            (s, w) => s + Number(w.budget_vol || 0),
            0,
          );
          const bCA = budgetWeeklyData.reduce(
            (s, w) => s + Number(w.budget_ca || 0),
            0,
          );
        ...
        // Forecast data
        if (
          Array.isArray(forecastWeeklyData) &&
          forecastWeeklyData.length > 0
        ) {
          const fVol = forecastWeeklyData.reduce(
            (s, w) => s + Number(w.forecast_vol || 0),
            0,
          );
          const fCA = forecastWeeklyData.reduce(
            (s, w) => s + Number(w.forecast_ca || 0),
            0,
          );
  ...
  const dCAN1 = useMemo(() => calcDelta(totalCA, n1CA), [totalCA, n1CA]);
  ...
  const getChartData = useCallback(
    (type: "volume" | "ca") => {
      ...
      const forecastData = data2026.map((r) => {
        const val =
          type === "volume"
            ? Number(r.total_forecast)
            : Number(r.total_forecast_ca);
        return isNaN(val) ? 0 : val;
      });

      const n1Data = data2026.map((r) => {
        const val =
          type === "volume" ? Number(r.total_n1) : Number(r.total_n1_ca);
        return isNaN(val) ? 0 : val;
      });
  ...
  const chartConfigVolume = useMemo(
    () => makeChartConfig(C.red),
    [makeChartConfig],
  );
  const chartConfigCA = useMemo(
    () => makeChartConfig(C.green),
    [makeChartConfig],
  );
      ...
      >
        ...
        <View style={sb.section}>
          ...
          <View style={[sb.item, sb.itemActive]}>
            ...
            <Text style={[sb.itemLabel, sb.itemLabelActive]}>
              Sales Dashboard
            </Text>
            <View style={sb.pip} />
          </View>
