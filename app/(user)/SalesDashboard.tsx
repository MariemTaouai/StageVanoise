...
/* ─── Sub-components ────────────────────────────────────────── */
const StatTile = ({ label, value, stripe }: {
...
export default function SalesDashboardScreen() {
  ...
  const [budgetMonthly, setBudgetMonthly] = useState<any[]>([]);

const getChartData = (type: "volume" | "ca") => {
  // Plus besoin de filtrer par année car le backend regroupe tout par mois
  const data2026 = [...monthlyHistory].sort((a: any, b: any) => Number(a.mois) - Number(b.mois));

  const nbMois = data2026.length || 1;
  const labels = data2026.length > 0 ? data2026.map((r: any) => MOIS[Number(r.mois) - 1]) : ["—"];

  const realData = data2026.map((r: any) =>
    type === "volume" ? Number(r.total_quantite) : Number(r.total_ca)
  );

  const budgetData = data2026.map((r: any) => 
    type === "volume" ? Number(r.total_budget) : Number(r.total_budget_ca)
  );

  const forecastData = data2026.map((r: any) => 
    type === "volume" ? Number(r.total_forecast) : Number(r.total_forecast_ca)
  );

  const n1Data = data2026.map((r: any) => 
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const [resSales, resBudget, resForecast, resYearly, resWeekly, resMonthly] =
          await Promise.all([
            ...
            fetch(`${API_URL}/api/sales/budget-monthly`),  // ← nouveau
    ...
    <SafeAreaView style={s.root}>
      ...
      <Animated.View style={[s.sidebar, { transform: [{ translateX: slideAnim }] }]}>
        ...
        <View style={sb.section}>
          ...
          {[
            { icon: "📊", label: "Sales Dashboard", active: true },
